    ...
    var body: some View {
        VStack(spacing: 0) {
            Picker("Kind", selection: $kind) {
                Text("Imports").tag(ImportExportService.Kind.imports)
                Text("Exports").tag(ImportExportService.Kind.exports)
            }
...
private struct CloudEntryRow: View {
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(expiryText)
                    .font(.caption2)
                    .foregroundStyle(expiryIsSoon ? .orange : .secondary)
            }
            Spacer()
    ...
    private var expiryText: String {
        guard let expiryDate else { return "Expiry unknown" }
